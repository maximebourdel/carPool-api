...
class MailManager
{
     ...
     */
    public static function sendEmail($template, $parameters, $to, $from, $fromName = null)
    {
    ...
    static function sendMailDemandeReservation ($controler, Reservation $newReservation){
        
        $message = (new \Swift_Message('Demande de reservation de '. $newReservation->getEmail()))
            ->setFrom('carpool@businessdecision.com')
            ->setTo($newReservation->getEmail())
            ->setBody(
                $controler->renderView(
        ...
        $controler->get('mailer')->send($message);
    }
}